...
class CompactDetailedReporter implements Reporter {
  ...
  /**
   * 로그 메시지에서 이모지를 심볼로 변환하고 색상을 적용하는 헬퍼 메서드
   */
  private processLogMessage(log: string): void {
    const cleanLog = log
      .replace('🚀', '▶')
      .replace('📱', '●')
      .replace('🧹', '◐')
  ...
  onBegin(_config: FullConfig, suite: Suite) {
    ...
    console.log(
      `${colorize('ℹ 총', 'cyan')} ${colorize(this.testStats.total.toString(), 'bright')}${colorize('개 테스트 실행 예정', 'cyan')}`
    );
    // 실제 실행되는 프로젝트만 표시
    ...
    console.log(
      `${colorize('ℹ 브라우저:', 'blue')} ${colorize(runningProjects.join(', ') || 'default', 'white')}`
    );
    console.log('');
    console.log(colorize('─'.repeat(60), 'gray'));
  ...
  onStepBegin(test: TestCase, _result: TestResult, step: TestStep) {
    if (process.env.SHOW_STEP_LOG !== 'true') {
      return;
    }
